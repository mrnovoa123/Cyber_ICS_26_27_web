...
function hero(){
  ...
  s.innerHTML = `
    ...
    <p class="sub">Diseño del entorno de prácticas, teoría por unidad y resolución de las 26 actividades del módulo. Cada actividad incorpora su tarea de laboratorio (con y sin IA), el riesgo que la IA introduce y el control que lo contiene. Curso de Especialización en Ciberseguridade — IES Chan do Monte (Marín), 2026/2027.</p>
    <div class="facts">
...
function labSection(){
  ...
  const vagrant = codeBlock({who:"Vagrantfile — levanta las tres VMs Linux", lines:'<span class="c"># Laboratorio MP5021 — red interna aislada 10.10.10.0/24</span>\nVagrant.configure(<span class="s">"2"</span>) <span class="k">do</span> |config|\n  config.vm.define <span class="s">"siem"</span> <span class="k">do</span> |siem|\n    siem.vm.box = <span class="s">"bento/ubuntu-22.04"</span>\n    siem.vm.network <span class="s">"private_network"</span>, ip: <span class="s">"10.10.10.10"</span>,\n      virtualbox__intnet: <span class="s">"labcyber-lan"</span>\n    siem.vm.network <span class="s">"private_network"</span>, ip: <span class="s">"192.168.56.10"</span>\n    siem.vm.provider <span class="s">"virtualbox"</span> <span class="k">do</span> |vb| vb.memory=4096; vb.cpus=2 <span class="k">end</span>\n  <span class="k">end</span>\n  config.vm.define <span class="s">"srvweb"</span> <span class="k">do</span> |web|\n    web.vm.box = <span class="s">"bento/ubuntu-22.04"</span>\n    web.vm.network <span class="s">"private_network"</span>, ip: <span class="s">"10.10.10.30"</span>,\n      virtualbox__intnet: <span class="s">"labcyber-lan"</span>\n    web.vm.provider <span class="s">"virtualbox"</span> <span class="k">do</span> |vb| vb.memory=2048; vb.cpus=1 <span class="k">end</span>\n  <span class="k">end</span>\n  config.vm.define <span class="s">"kali"</span> <span class="k">do</span> |kali|\n    kali.vm.box = <span class="s">"kalilinux/rolling"</span>\n    kali.vm.network <span class="s">"private_network"</span>, ip: <span class="s">"10.10.10.100"</span>,\n      virtualbox__intnet: <span class="s">"labcyber-lan"</span>\n    kali.vm.network <span class="s">"private_network"</span>, ip: <span class="s">"192.168.56.100"</span>\n    kali.vm.provider <span class="s">"virtualbox"</span> <span class="k">do</span> |vb| vb.memory=4096; vb.cpus=2 <span class="k">end</span>\n  <span class="k">end</span>\n<span class="k">end</span>'});
  const setup = codeBlock({who:"siem — Wazuh all-in-one + Suricata", lines:'<span class="c"># Instalar Wazuh (SIEM/XDR); imprime la contraseña de admin</span>\ncurl -sO https://packages.wazuh.com/4.x/wazuh-install.sh\nsudo bash ./wazuh-install.sh -a\n<span class="c"># Panel: https://192.168.56.10  (usuario admin)</span>\nsudo apt install -y suricata &amp;&amp; sudo suricata-update'})
    + codeBlock({who:"kali — comprobar que la red está aislada", lines:'<span class="a">ping</span> -c2 10.10.10.10        <span class="c"># llega al SIEM</span>\n<span class="a">nmap</span> -sn 10.10.10.0/24     <span class="c"># descubre máquinas vivas</span>\n<span class="a">ping</span> -c2 8.8.8.8           <span class="c"># en modo ataque DEBE fallar</span>'});
  s.innerHTML = `
    ...
    <div class="note fix">${ICON.fix}<div>Versión completa y actualizada (4 VMs, scripts de configuración y verificación) en el <a href="#vagrant"><b>Manual Vagrant</b></a>. El fragmento de abajo es solo el esquema inicial.</div></div>
    <h3>Vagrantfile base</h3>${vagrant}
    <h3>Puesta a punto — comandos clave</h3>${setup}`;
  return s;
}
...
function vagrantSection(){
  ...
  s.innerHTML = `
    ...
    <div class="note">${ICON.info}<div>Todos los ficheros del laboratorio aparecen completos más abajo, con botón <b>copiar</b>. Guárdalos en <span class="mono">labcyber/</span> con la estructura del apartado 4 (o usa el paquete <span class="mono">labcyber.zip</span> que acompaña a este material).</div></div>
...
function activityCard(a, color){
  ...
  if(a.ia){
    ...
    if(a.ia.risk) ia += `<div class="line risk"><b>Riesgo ·</b> ${a.ia.risk}</div>`;
    if(a.ia.ctrl) ia += `<div class="line ctrl"><b>Control ·</b> ${a.ia.ctrl}</div>`;
    if(a.ia.labId && typeof LAB_TASKS!=="undefined" && LAB_TASKS[a.ia.labId]){
      const L = LAB_TASKS[a.ia.labId];
      ia += `<div class="labia"><div class="lh">Laboratorio integrado</div><h4 style="margin-top:6px">${L.t}</h4><p>${L.escenario}</p><div class="lh">Elementos necesarios</div>${list(L.elementos)}<div class="lh">Resolución paso a paso</div><ol>${L.pasos.map(x=>`<li>${x}</li>`).join("")}</ol><div class="deliver"><b>Entregable ·</b> ${L.entregable}</div></div>`;
    }
    ia += `</div>`;
  }
  let fixes = "";
  if(a.fixes) fixes = `<div class="note fix">${ICON.fix}<div><b>Corregido respecto al material original:</b><ul style="margin:6px 0 0">${a.fixes.map(x=>`<li>${x}</li>`).join("")}</ul></div></div>`;
  let q = "";
  if(a.q) q = `<div class="qbox"><div class="qh">${ICON.q}Decisiones tomadas / abiertas</div><ul>${a.q.map(x=>`<li>${x}</li>`).join("")}</ul></div>`;
...
/* ---------- CIERRE ---------- */
function usoSection(){
  const s = el("section"); s.id="uso"; s.setAttribute("data-ia","1");
  s.innerHTML = `
    <div class="sec-head"><span class="tag" style="background:var(--attack)">AULA</span><h2>Cuando el problema es el uso</h2></div>
    <p class="sec-meta">Incidente transversal · mal uso de la IA por parte del alumnado</p>
    <p class="lead">Va a ocurrir, así que conviene tratarlo como un incidente más, con procedimiento escrito.</p>
    <h4>Señales</h4><ul><li>Entregas sin errores pero sin criterio.</li><li>Bitácora de prompts rellenada a posteriori.</li><li>El alumno no sabe explicar su propio trabajo.</li><li>Estructuras de texto idénticas entre grupos.</li></ul>
    <h4>Procedimiento en cuatro pasos</h4>
    <ol>
      <li><b>Defensa oral de cinco minutos.</b> No se acusa: se pregunta («explícame por qué elegiste esta medida y no la siguiente de tu lista»). Resuelve el 90 % de los casos sin conflicto.</li>
      <li><b>Reconducción, no sanción.</b> Se devuelve con una consigna concreta: «vuelve a entregarlo con las tres fuentes verificadas y con lo que descartaste de la IA».</li>
      <li><b>Rediseño de la tarea.</b> Si medio grupo entrega lo mismo, el problema es el enunciado. Los enunciados anclados al laboratorio propio —salidas reales de <em>sus</em> VM, sus IP, sus capturas— son casi inmunes.</li>
      <li><b>Registro.</b> Se anota en la tabla de observación como evidencia del proceso, que es justo el instrumento que prevé la programación.</li>
    </ol>`;
  return s;
}
function resumenSection(){
  const s = el("section"); s.id="resumen"; s.setAttribute("data-ia","1");
  const rows = [
    ["UD1","Generar casos, público simulado, checklist de auditoría","Alucinación normativa","Normativa + mural con casos tachados"],
    ["UD2","Triaje de alertas, reglas Sigma, plan OSINT","<b>Prompt injection vía log</b>","Ficha del propio envenenamiento"],
    ["UD3","Desofuscación, ATT&CK, hipótesis alternativas","Atribución alucinada; evidencia fuera del lab","Informe: confirmado / probable / hipótesis"],
    ["UD4","Playbooks, árbol de escalado, post-mortem","Script destructivo; consejo ilegal","Playbook ejecutado por otro grupo"],
    ["UD5","Redacción multi-audiencia, rueda de prensa","Plazo alucinado; datos reales en el comunicado","Tabla de plazos verificada + doble redacción"]
  ].map((r,i)=>`<tr><td class="mono" style="color:${UNITS[i].color};font-weight:600">${r[0]}</td><td>${r[1]}</td><td>${r[2]}</td><td>${r[3]}</td></tr>`).join("");
  s.innerHTML = `
    <div class="sec-head"><span class="tag" style="background:var(--ia)">MAPA</span><h2>Resumen operativo</h2></div>
    <p class="sec-meta">Una fila por unidad</p>
    <div class="tbl-wrap"><table><thead><tr><th>UD</th><th>Uso principal de la IA</th><th>Fallo a provocar</th><th>Producto de defensa</th></tr></thead><tbody>${rows}</tbody></table></div>
    <p class="lead">Al terminar el módulo, el alumnado debería poder decir con precisión en qué paso de su trabajo la IA le ayudó y en qué paso le habría hecho fallar.</p>`;
  return s;
}
...
function footer(){
  const f = el("footer");
  f.innerHTML = `
    <p>Material didáctico del módulo <b>MP5021 · Incidentes de Ciberseguridade</b> (Curso de Especialización en Ciberseguridade en Contornos das TI). Basado en la programación LOOIFP del IES Chan do Monte (Marín), curso 2026/2027, en los apuntes y tareas del aula, y en guías de INCIBE-CERT y CCN-CERT.</p>
    <p class="legal">Todas las herramientas ofensivas se usan <b>exclusivamente dentro de la red aislada del laboratorio</b> y sobre activos ficticios. OSINT y phishing solo contra objetivos de prueba propios, nunca contra terceros reales. Los títulos de las actividades se mantienen en gallego, tal como figuran en la programación oficial; el desarrollo está en castellano.</p>`;
  return f;
}
...
function buildNav(){
  ...
  h += `<li class="grp">Cierre</li>
    <li data-ia="1"><a href="#uso" class="udh" style="--udc:var(--attack)"><span class="n">AULA</span>Uso indebido de la IA</a></li>
    <li data-ia="1"><a href="#resumen" class="udh" style="--udc:var(--ia)"><span class="n">MAPA</span>Resumen operativo</a></li>
